...
DEVICE OFFLINE – OL
USE: Occurs when a station, trunk, hunt group, trunk group, or voice mail application goes offline or
...
Table 11: Device Offline Conditions
DEVICE OFFLINE IF
Keyset The keyset or keyset card is unplugged.
Single-Line Set The SLC is unplugged.
Trunk
The trunk card is unplugged or Call Processing knows
...
Hunt Group
The hunt group has no available members, no
...
NOTES: (Applies to all versions.) During a resync, the system issues this event only if the device is
...
MONITOR TYPE: Device (Requires the offline <filter_list> bit enabled [bit 0])
SYNTAX
(V < 08.10): OL,<Resync_Code>,<Mon_Cross_Ref_ID>,<Extension>,
...
Table 12: OL – Device_Type Values
VALUE <DEVICE_TYPE>
...
EXAMPLES: Trunk 94017 transitions to online.
...
GATEWAY IMPACTS: The CT Gateway allows an application to enable the OL status bit through the Monitor
